...
// --------------------
  ...
  const BRAND_ICON_SVG = {
    ...
    ya: `<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><rect width="64" height="64" rx="18" fill="#fff"/><path d="M35.4 11C25.2 11 19 16.4 19 25.3c0 6.2 3.2 10.9 8.8 13.2L20.8 53h8.8l8.1-13.1h4.1V53h7.6V11h-14zm6.4 22.5h-5.9V17.7h5.9c4 0 6.5 3.1 6.5 7.8 0 4.8-2.5 8-6.5 8z" fill="#FC3F1D"/></svg>`,
    gm: `<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><rect width="64" height="64" rx="18" fill="#fff"/><path d="M32 8c-9.9 0-18 8-18 17.9 0 12.9 15.1 27.9 16.8 29.6a1.7 1.7 0 0 0 2.4 0C34.9 53.8 50 38.8 50 25.9 50 16 41.9 8 32 8z" fill="#EA4335"/><path d="M32 8c-9.9 0-18 8-18 17.9 0 3.7 1.3 7.8 3.7 12.1L32 24.4V8z" fill="#4285F4"/><path d="M32 24.4 46.3 38c2.4-4.3 3.7-8.4 3.7-12.1C50 16 41.9 8 32 8z" fill="#FBBC04"/><path d="M17.7 38c4.2 7.5 11.3 14.4 13.1 16.1a1.7 1.7 0 0 0 2.4 0c1.8-1.7 8.9-8.6 13.1-16.1L32 24.4 17.7 38z" fill="#34A853"/><circle cx="32" cy="25.9" r="7.3" fill="#fff"/><circle cx="32" cy="25.9" r="4.1" fill="#4285F4"/></svg>`,
...
function brandIconHtml(taskOrType, sizePx = 38) {
  ...
  function renderTasks() {
    ...
    list.forEach(t => {
      ...
      card.className = "card task-card-premium" + (isMy ? " is-owner" : " is-worker");
      ...
      if (isMy) {
        ...
        const leftLabel = left > 999 ? "999+" : String(left);
        card.innerHTML = `
          <div class="task-premium-glow"></div>
          <div class="task-premium-row">
            <div class="task-premium-main">
              <div class="brand-box premium-brand-box" style="width:52px; height:52px; font-size:18px;">${brandIconHtml(t, 40)}</div>
              <div class="task-premium-copy">
                <div class="task-premium-title">${safeText(t.title || "Задание")}</div>
                <div class="task-premium-sub">${safeText(subtypeText)}</div>
                <div class="task-premium-meta">
                  <span class="task-meta-chip">Доступно: ${leftLabel}</span>
                  ${topActive ? `<span class="task-meta-chip hot">🔥 Топ</span>` : ``}
                </div>
              </div>
            </div>
            <div class="task-premium-reward">+${fmtRub(reward)}</div>
          </div>`;
