...
<svg xmlns="http://www.w3.org/2000/svg" width="128" height="128" viewBox="0 0 128 128">
  ...
  // --------------------
  // Local click marker (fallback if network blocks /api/task/click)
  // --------------------
  function setLocalTaskClick(taskId){
    const k = "rc_clicked_task_" + String(taskId);
    const ts = Date.now();
    state.localClicks[String(taskId)] = ts;
    try { sessionStorage.setItem(k, String(ts)); } catch(e){}
  }
  function getLocalTaskClick(taskId){
    const id = String(taskId);
    if (state.localClicks[id]) return state.localClicks[id];
    const k = "rc_clicked_task_" + id;
    try {
      const v = sessionStorage.getItem(k);
      const n = v ? parseInt(v, 10) : 0;
      if (n) { state.localClicks[id] = n; return n; }
    } catch(e){}
    return 0;
  }
  function hasRecentLocalTaskClick(taskId, maxMs){
    const ts = getLocalTaskClick(taskId);
    if (!ts) return false;
    return (Date.now() - ts) <= (maxMs || (6 * 3600 * 1000));
  }
...
if (!list.length) {
  ...
  function openTaskDetails(task) {
    ...
    if (a) {
      ...
      a.onclick = async (e) => {
        ...
        // mark locally (so submit can recover if request fails)
        try { setLocalTaskClick(task.id); } catch (e_local) {}
  ...
  async function submitTaskManual(task) {
    ...
    try {
      ...
      let res;
      try {
        res = await apiPost("/api/task/submit", {
          task_id: String(task.id),
          proof_text: proofText,
          proof_url: proofUrl,
        });
      } catch (errSubmit) {
        const msg = String((errSubmit && errSubmit.message) || errSubmit || "");
        // If backend says "you didn't click", but user actually pressed the link button,
        // try to re-send /api/task/click and retry submit once.
        if (msg.includes("Сначала нажми") && hasRecentLocalTaskClick(task.id) ) {
          try { await apiPost("/api/task/click", { task_id: String(task.id) }); } catch(e2) {}
          res = await apiPost("/api/task/submit", {
            task_id: String(task.id),
            proof_text: proofText,
            proof_url: proofUrl,
