...
// --------------------
  ...
  function renderTaskInstructionHtml(task) {
    ...
    const heading = mode === "per_item"
      ? "Тексты отзыва от заказчика"
      : "Готовый текст отзыва от заказчика";
    const items = reviewTexts.map((text, idx) => `<div class="review-text-item"><span class="review-text-index">${mode === "per_item" ? idx + 1 : '★'}</span><span>${safeText(text)}</span></div>`).join("");
  ...
  function syncReviewTextsHint() {
    ...
    if (mode === "single") {
      helper.textContent = "Этот текст увидят все исполнители вместо общей инструкции.";
    } else if (mode === "per_item") {
      helper.textContent = `Каждая строка — отдельный текст отзыва. Сейчас заказано ${qty} шт.; лучше указать до ${qty} вариантов.`;
    } else {
      helper.textContent = "Можно оставить общий текст либо задать один шаблон/отдельные тексты по строкам.";
    }
  }
