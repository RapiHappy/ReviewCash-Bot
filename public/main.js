  ...
  async function submitTaskAuto(task) {
    ...
    try {
      ...
      if (res && res.ok) {
        ...
        const earned = Number(res.earned ?? task.reward_rub ?? 0);
        const xpAdded = Number(res.xp_added || 0);

        // Optimistic UI update (so XP shows immediately even if sync fails)
        try {
          if (!state.balance) state.balance = { rub_balance: 0, stars_balance: 0, xp: 0, level: 1 };
          if (!Number.isNaN(earned)) state.balance.rub_balance = Number(state.balance.rub_balance || 0) + earned;
          if (xpAdded > 0) {
            state.balance.xp = Number(state.balance.xp || 0) + xpAdded;
            state.balance.level = Math.floor(Number(state.balance.xp || 0) / 100) + 1;
          }
          // If backend sent full balance, trust it
          if (res.balance && typeof res.balance === "object") state.balance = res.balance;
          renderHeader();
          renderProfile();
        } catch (e) {}
        ...
        tgHaptic("success");
        tgAlert(
          "Готово! Начислено: +" + fmtRub(earned) + (xpAdded ? (" и +" + xpAdded + " XP") : ""),
          "success",
          "Награда"
        );

        // Best-effort full sync
